...
void wrp_i2c_slave_rx_init_DMA(wrp_i2c_slave_header*   header);
...
void wrp_i2c_slave_rx_init_DMA ( wrp_i2c_slave_header* header) {
	...
	header->dmaRxChanel->CPAR = &header->hDevice->DR;
	header->dmaRxChanel->CMAR = header->i2cRxSlaveBuffer;
     ///----------I2C---initialization
	 //enable own address
...
/*3) Initialize an instance of a structure,
 ...
 to restore options in DMA after transactions*/
    ...
    i2cSlaveHeader.i2cTxSlaveBuffer = 0; //Tx bufer
    i2cSlaveHeader.i2cSlaveRxPacketLength = 128;
    i2cSlaveHeader.i2cSlaveTxPacketLength = 0;
  ...
  //SLAVE
	if (i2cSlaveHeader.hDevice->SR1 & I2C_SR1_ADDR) {
		...
		if (i2cSlaveHeader.hDevice->SR2 & I2C_SR2_TRA) {
				  ///slave transmitter mode (not realized here)
		} else {
					//slave receiver mode
					//restore data length
